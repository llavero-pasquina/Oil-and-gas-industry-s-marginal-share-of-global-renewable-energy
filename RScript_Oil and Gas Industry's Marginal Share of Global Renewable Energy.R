...
#Llavero-Pasquina, M., Bontempi, A. 2025. "Oil and gas industry’s marginal share of global renewable energy". Nature Sustainability
...
setwd("G:/My Drive/Research/ZZ.Submitted/Part of the Solution/Submission NS/Review #1/Data Analysis/Subsidiaries")

#Aggregate all company trees, and list all subsidiaries in a single column
Subsidiaries_files <- list.files("G:/My Drive/Research/ZZ.Submitted/Part of the Solution/Submission NS/Review #1/Data Analysis/Subsidiaries")[-1]
Subsidiaries <- NULL
...
setwd("G:/My Drive/Research/ZZ.Submitted/Part of the Solution/Submission NS/Review #1/Data Analysis/")
...
setwd("G:/My Drive/Research/ZZ.Submitted/Part of the Solution/Submission NS/Review #1/Data Analysis/Sisters")

Sister_files <- list.files("G:/My Drive/Research/ZZ.Submitted/Part of the Solution/Submission NS/Review #1/Data Analysis/Sisters")
Sister <- NULL
...
setwd("C:/Users/Usuario/Desktop/Marcel/EJAtlas/Research/False Solutions")
...
Companies <- read.csv("Companies Master.csv", dec = ",", encoding = "UTF-8")
Companies$Share <- as.numeric(Companies$Share)

#When ownership share is unknown assume it is 1
Companies$Share[is.na(Companies$Share)] <- 1

#Global oil and gas production in 2023 was 94292 kb/d oil + 4048.6 bcm gas * 6429 kboe/bcm / 365 days = 165602.8 kboe/d
...
#Top 250 companies cover 88.3% of global oil and gas production
gogel <- read_xlsx("urgewald GOGEL 2024 V1.1.xlsx.", sheet = 2)
gogel <- gogel[-which(gogel$...8 %in% "Subsidiary"),]
sum(as.numeric(Companies$Hydrocarbons.Production.in.2022..Oil..Gas..Condensate..NGL.), na.rm = T)/
  sum(as.numeric(gogel$...13[6:nrow(gogel)]), na.rm = T)
#Top 250 companies responsible for 96.2% of production in GOGEL
table(Companies$Company.Type)
...
Wind <- read_xlsx("Global-Wind-Power-Tracker-February-2025.xlsx", sheet = 2) #Only utility scale >10MW
Solar <- read_xlsx("Global-Solar-Power-Tracker-February-2025.xlsx", sheet = 2) #Only utility scale >20MW, >10MW in Arab speaking countries
Hydro <- read_xlsx("Global-Hydropower-Tracker-April-2024.xlsx", sheet = 2)
Geo <- read_xlsx("Geothermal-Power-Tracker-March-2025-Final.xlsx", sheet = 2) 
#Bio <- read_xlsx("Global-Bioenergy-Power-Tracker-GBPT-September-2024.xlsx", sheet = 2) 
...
#colnames(Bio) <- gsub("Country/Area","Country",colnames(Bio))
#colnames(Bio)[grep("Owner",colnames(Bio))] <- c("Owner","Owner Name(s) in Local Language / Script")
#colnames(Bio)[grep("Operator",colnames(Bio))] <- c("Operator","Operator Name(s) in Local Language / Script")
...
colnames(Hydro) <- gsub("Country 1","Country",colnames(Hydro))
colnames(Hydro) <- gsub("Region 1","Region",colnames(Hydro))

#Merge GEM trackers
FF<-NULL
#FF <- Bio[,which(colnames(Bio) %in% c("Country","Project Name","Capacity (MW)","Status","Operator","Owner","Latitude","Longitude","Region","GEM location ID","GEM phase ID","Wiki URL"))]
...
FF$Owner[which(is.na(FF$Owner))] <- FF$Operator[which(is.na(FF$Owner))]
...
FF$Owner[which(FF$Owner %in% "Glennmont Partners; CEF 2 Wind Energy")] <- "Eni Plenitude SpA"
...
FF$Owner[which(FF$Owner %in% "Galp Energia SGPS SA ; Actividades de Construcción y Servicios SA")] <- "Actividades de Construccion y Servicios [25%]; Galp ACS [75%]"
...
#Vargronn (Eni) acquires EngW Baltic Wind Farm
#https://www.vargronn.com/news/vargronn-completes-acquisition-of-baltic-2-offshore-wind-farm
FF$Owner[which(FF$Owner %in% "Macquarie Capital; EnBW")] <- "Vargronn"

#HitecVision acquires Wind Farm
#https://hitecvision.com/news/aneo-acquires-its-first-wind-farm-in-finland/
FF$Owner[which(FF$`Project Name` %in% "Kokkoneva (Greencoat) wind farm")] <- "HitecVision AS; TronderEnergi AS"

#TotalEnergies buys 51% stake in Bujagali hydroelectric plant
#https://totalenergies.com/news/press-releases/renewables-totalenergies-acquires-portfolio-hydropower-projects-africa-deploy
FF$Owner[which(FF$`Project Name` %in% "Bujagali hydroelectric plant")] <- "TotalEnergies SE [28.3%]; Others [49%]"

#Eni Acquires 49% Sandrini Solar Project from EDP Renewables
#https://renovables.eniplenitude.es/actualidad/plenitude-firma-acuerdo-con-edp-renewables/
FF$Owner[which(FF$`Project Name` %in% "Sandrini Solar Project")] <- "EDPR Renewable Energy Sources of Greece 1 Sole Proprietorship [51%]; Eni SpA [49%]"
...
#Pan American Energy owns Novo Horizonte
#https://www.pan-energy.com/en/renewable/
FF$Owner[which(FF$`Project Name` %in% "Novo Horizonte wind farm")] <- "Pan American Energy Group SL"

#Chubut Norte III & IV are shared between Genneia and Pan American Energy
#https://www.pan-energy.com/en/renewable/
FF$Owner[which(FF$`GEM phase ID` %in% c("G100000907010","G100000907011"))] <- "Genneia SA; Pan American Energy Group SL"

#Hywind Tampen
#https://www.power-technology.com/data-insights/power-plant-profile-hywind-tampen-norway/
FF$Owner[which(FF$`Project Name` %in% "Hywind Tampen Floating Offshore wind farm")] <- "Equinor [41%]; DEA Norge [9.83%]; ExxonMobil Exploration and Production Norway [9.83%]; Inpex Idemitsu Norge [9.83%]; OMV (Norge) [9.83%]; Petoro [9.83%]; Vaar Energi [9.83%]"

#Moray East Offshore wind farm project
#https://www.inpex.com/english/business/project/moray-east.html
FF$Owner[which(FF$`Project Name` %in% "Moray East Offshore wind farm")] <- "INPEX Europe Limited [16.7%]; Ocean Winds [40.0%]; Equitix [26.6%]; Kansai Electric Power [10.02%]; Mitsubishi HC Capital [6.68%]"

#Enel Green Power Australia 50% owned by INPEX
#https://www.inpex.com/english/business/project/bungala.html
FF$Owner[which(FF$Country %in% "Australia")] <- gsub("Enel Green Power SpA","Enel Green Power Australia",FF$Owner[which(FF$Country %in% "Australia")])

#Luchterduinen offshore
#https://www.inpex.com/english/business/project/luchterduinen.html
FF$Owner[grep("Luchterduinen",FF$`Project Name`)] <- "Eneco [50%]; INPEX Europe Limited [50%]"

#Sarulla Geothermal Share Break-down
#https://www.inpex.com/english/business/project/sarulla.html
FF$Owner[grep("Sarulla",FF$`Project Name`)] <- "INPEX Geothermal Sarulla [18.2525%]; Kyushu Electric Power [25.0%]; Itochu [25.0%]; Medco Energy International [18.9975%]; Ormat Technologies [12.75%]"

#Hai Long Offshore Acquisition
#https://www.power-technology.com/news/northland-stake-hai-long-gentari/
FF$Owner[grep("Hai Long",FF$`Project Name`)] <- "Mitsui & Co Ltd [20%]; Northland Power INC [30.6%]; Gentari [29.4%]; Yushan Energy Taiwan [20%]"

#Niva Cascade - TGC-1 - Gazprom
#https://www.tgc1.ru/en/production/production-complex/tgc-1-in-murmansk-oblast/
FF$Owner[grep("TGC-2|TGC-3",FF$Owner)] <- "PJSC TGC-1 (Territorial generating company number 1)"
FF$Owner[which(FF$`Project Name` %in% "Imatra hydroelectric plant")] <- "PJSC TGC-1 (Territorial generating company number 1)"

#Aktina - Ramsey Solar Farm - Tokyo Gas
#https://www.tokyo-gas.co.jp/en/IR/library/pdf/tekijikaiji/20200729-04e.pdf
FF$Owner[which(FF$`Project Name` %in% "Ramsey solar farm")] <- "Tokyo Gas Co Ltd"

#Peralta Wind Farm - Pluspetrol
#https://www.pluspetrol.net/en/articulo/pluspetrol-acquires-its-first-renewable-asset-in-uruguay
FF$Owner[which(FF$`Project Name` %in% "Peralta wind farm")] <- "PlusPetrol SA"

#Oaxaca Wind Farm - International Petroleum
#https://www.international-petroleum.com/download/sustainability-report-2023
FF$Owner[which(FF$`GEM phase ID` %in% "G100000906481")] <- "International Petroleum Corporation (IPC)"

#Orosi Wind Farm - International Petroleum
#https://www.international-petroleum.com/download/sustainability-report-2023
FF$Owner[which(FF$`Project Name` %in% "Orosi wind farm")] <- "International Petroleum Corporation (IPC)"

#Dagget Solar - TotalEnergies
#https://investor.clearwayenergy.com/static-files/0ff4ee5c-34ed-4e1a-b1aa-d9d7812df544
FF$Owner[which(FF$`Project Name` %in% "Daggett solar farm")] <- "Clearway Energy Group LLC [25%]; Various [75%]"

#Arica Solar - TotalEnergies
#https://investor.clearwayenergy.com/static-files/0ff4ee5c-34ed-4e1a-b1aa-d9d7812df544
FF$Owner[which(FF$`Project Name` %in% "Arica Solar")] <- "Clearway Energy Group LLC [40%]; Various [60%]"

#Rosamond Solar - TotalEnergies
#https://investor.clearwayenergy.com/static-files/0ff4ee5c-34ed-4e1a-b1aa-d9d7812df544
FF$Owner[grep("Rosamond",FF$`Project Name`)] <- "Clearway Energy Group LLC [50%]; Various [50%]"

#Texas Solar Nova - TotalEnergies
#https://investor.clearwayenergy.com/static-files/0ff4ee5c-34ed-4e1a-b1aa-d9d7812df544
FF$Owner[which(FF$`Project Name` %in% "Texas Solar Nova")] <- "Clearway Energy Group LLC [50%]; Verizon [50%]"

#Victory Pass - TotalEnergies
#https://investor.clearwayenergy.com/static-files/0ff4ee5c-34ed-4e1a-b1aa-d9d7812df544
FF$Owner[which(FF$`Project Name` %in% "Victory Pass Solar")] <- "Clearway Energy Group LLC [40%]; Various [60%]"

#Meskite Wind Nova - TotalEnergies
#https://investor.clearwayenergy.com/static-files/0ff4ee5c-34ed-4e1a-b1aa-d9d7812df544
FF$Owner[which(FF$`Project Name` %in% c("Mesquite Sky wind farm","Mesquite Star wind farm"))] <- "Clearway Energy Group LLC [50%]; Various [50%]"

#Cottonwood Bayou Solar - TotalEnergies
#https://investor.clearwayenergy.com/static-files/0ff4ee5c-34ed-4e1a-b1aa-d9d7812df544
#https://corporate.totalenergies.us/discover-our-solar-projects
FF$Owner[which(FF$`Project Name` %in% "Cottonwood Bayou Solar")] <- "TotalEnergies SE"

#Black Rock wind farm - TotalEnergies
#https://investor.clearwayenergy.com/static-files/0ff4ee5c-34ed-4e1a-b1aa-d9d7812df544
FF$Owner[which(FF$`Project Name` %in% "Black Rock wind farm")] <- "Clearway Energy Group LLC [50%]; Various [50%]"

#Sakata Port Solar - ENEOS
#https://www.eneos-re.com/english/business/list/
FF$Owner[which(FF$`Project Name` %in% c("Saka Port Mega Solar Park","Rokunohe Solar Park",
                                        "Fukushima Prefecture Iwaki solar project XIII",
                                        "Shirakawa Solar Park",
                                        "Nishinosata Solar Park",
                                        "Mito New Town Mega Solar Park",
                                        "Jre Aso Takamori Solar Power Plant",
                                        "Jre Satsuma solar power plant",
                                        "Jre Tsuchiura Solar Power Plant",
                                        "Jre Yamato Takamori Solar Power Plant"
                                        ))] <- "ENEOS Corp"

#GK JRE - ENEOS
#https://www.eneos-re.com/english/business/list/
FF$Owner[grep("GK JRE",FF$Owner)] <- "ENEOS Corp"

#Renopool Solar Plants  - Eni
#https://www.eni.com/content/dam/enicom/documents/eng/reports/2024/ar-2024/Annual-Report-2024.pdf
FF$Owner[which(FF$`Project Name` %in% "Renopool solar farm")] <- "Eni Plenitude SpA"
FF$Status[which(FF$`Project Name` %in% "Renopool solar farm")] <- "operating"

#Caparacena Solar Plants  - Eni
#https://www.eni.com/content/dam/enicom/documents/eng/reports/2024/ar-2024/Annual-Report-2024.pdf
FF$Owner[which(FF$`Project Name` %in% "Caparacena 400 solar farm")] <- "Eni Plenitude SpA"
FF$Status[which(FF$`Project Name` %in% "Caparacena 400 solar farm")] <- "operating"

#Borgia Wind Farm  - Eni
#https://www.eni.com/en-IT/media/press-release/2024/07/cs-plenitude-completa-realizzazione-parco-eolico-calabria.html
FF$Owner[which(FF$`Project Name` %in% "Borgia wind farm")] <- "Eni Plenitude SpA"
FF$Status[which(FF$`Project Name` %in% "Borgia wind farm")] <- "operating"

#Bonete Solar Farm - Eni
#https://www.eleconomista.es/energia/noticias/12352490/07/23/la-italiana-eni-crece-en-espana-con-la-compra-a-la-israeli-helios-de-sus-plantas-solares.html
FF$Owner[which(FF$`Project Name` %in% "Bonete solar farm")] <- "Eni Plenitude SpA"

#Badamsha wind farm - Eni
#https://www.eni.com/en-IT/actions/global-activities/kazakhstan/badamsha.html
FF$Owner[which(FF$`Project Name` %in% "Badamsha wind farm")] <- "Eni Plenitude SpA"

#Bluebell Solar - Eni
#https://www.eni.com/content/dam/enicom/documents/eng/reports/2021/Fact-Book-2021-eng.pdf
FF$Owner[which(FF$`Project Name` %in% "Bluebell Solar")] <- "Eni Plenitude SpA"

#Delete all power plants without owner or operator
table(FF$Technology[(which(is.na(FF$Owner)))])
length(which(is.na(FF$Owner)))
#5840 (11%) plants without owner or operator
FF0 <- FF
FF <- FF[-which(is.na(FF$Owner)),] 

#### 7. Identify Solar, Wind, and Offsets projects owned by fossil fuel companies (Table S5) ####

Companies$Search[grep("Vargronn",Companies$Search)] <- "Vargronn(Vårgrønn)"
Companies$Search[grep("Vaar",Companies$Search)] <- "Vaar(Vår)"
Companies$Search[grep("Compania Espanola",Companies$Search)] <- "Compañía Española de Petróleos(Cepsa|Moeve)"

#Create a dataframe that compiles all projects owned by fossil fuel companies
FF250 <- NULL

#WARNING! Expect long computation time!!
pb <- txtProgressBar(min = 0, max = nrow(Companies), style = 3)
...
#FF250 <- FF250[-which(FF250$Company.Type %in% ""),]
...
Parents <- as.data.frame(table(FF250$Parent))
table(FF250$Company.Type)
...
Bycountry <- FF250 %>% group_by(Country) %>% summarise(EqCapacity = sum(ParentEqCap, na.rm = TRUE),
                                                       Projects = n())

Bytype <- FF250 %>% group_by(Company.Type) %>% summarise(EqCapacity = sum(ParentEqCap, na.rm = TRUE),
                                                    Projects = n())

Bystatus <- FF250 %>% group_by(Status) %>% summarise(EqCapacity = sum(ParentEqCap, na.rm = TRUE),
                                                         Projects = n())

Bytypestatus <- FF250 %>% group_by(Company.Type,Status) %>% summarise(EqCapacity = sum(ParentEqCap, na.rm = TRUE),
                                                         Projects = n())

Bytechnology <- FF250 %>% group_by(Technology) %>% summarise(EqCapacity = sum(ParentEqCap, na.rm = TRUE),
                                                     Projects = n())

Bytypetechnology <- FF250 %>% group_by(Company.Type,Technology) %>% summarise(EqCapacity = sum(ParentEqCap, na.rm = TRUE),
                                                             Projects = n())
...
BytechnologystatusFF <- FF0 %>% group_by(Technology,Status) %>% summarise(EqCapacity = sum(`Capacity (MW)`, na.rm = TRUE),
                                                                         Projects = n())
...
Piecharts_table <- b
write.csv(Piecharts_table,"Piecharts_GEM.csv")
...
#IRENA Renewable Capacity Statistics 2024: https://www.irena.org/Publications/2025/Mar/Renewable-capacity-statistics-2025
#Onshore wind: 1053403MW
#Offshore wind: 79434MW
#Solar: 1865490MW
#Hydroelectricity: 1425374MW
#Geothermal: 15427MW
#Total: 4439128MW
IRENA <- cbind.data.frame(c(levels(as.factor(Operating$Technology)),"Total"),c(15427,1425374,79434,1053403,1865490,4439128))
...
#Operating <- Operating[-which(Operating$Company.Type %in% "Sister"),]
Operating$Status <- as.factor(Operating$Status)
Operating$Status <- droplevels(Operating$Status)
Operating$Status <- droplevels(Operating$Company.Type)
...
PiechartsIRENA <- ggplot(Operating, aes(x="", y=EqCapacity, fill=Company.Type)) +
  ...
  scale_fill_manual(values = c("#131515","#7c7c7c","#bdbdbd","#3d8432","#9fe298ff"))+
  cleanplot
PiechartsIRENA
#Export and edit in Adobe Illustrator to make Figure 1a
ggsave(PiechartsIRENA, file="PiechartsIRENA.svg")
write.csv(Operating,"Piecharts_IRENA.csv")
...
b <- unique(Bycompanies$Parent) #51 Parents
...
Bycompanies[sapply(Bycompanies, is.factor)] <- lapply(Bycompanies[sapply(Bycompanies, is.factor)], 
                                                      as.character)
#Mitsubishi acquired a majority stake in Nexamp (assumed 0.51%) which owns hundreds of smaller scale solar projects 
#that are under the threshold of inclusion in Global Energy Monitor. 
#Nexamp reports 1.1GW (https://www.nexamp.com/), hence we add 550 MW of distributed.
Bycompanies <- rbind.data.frame(Bycompanies, c("Distributed","Mitsubishi Corporation","operating","Acquisition",550,NA))
...
Bycompanies <- rbind.data.frame(Bycompanies, c("Distributed","Eni SpA","operating","Subsidiary",541,NA))
#Through Amplus Solar it has an extensive network of distributed solar
#power plants that are below the GEM threshold totalling 1.1GW 
#(https://www.careratings.com/upload/CompanyFiles/PR/202403140357_Amplus_Solar_Power_Private_Limited.pdf). 
#0.420 GW from Amplus are already accounted for in GEM, 
#so we include 0.680 GW of distributed power.
Bycompanies <- rbind.data.frame(Bycompanies, c("Distributed","Petroliam Nasional Bhd (Petronas)","operating","Acquisition",680,NA))
...
#ENEOS has a cummulative 340MW of solar power plants under GEM threshold (>20MW) and 12MW of wind under GEM threshold (>10MW). We add these as distributed. We apply the solar capacity factor to the distributed capacity.
#https://ssl4.eir-parts.net/doc/5020/ir_material_for_fiscal_ym9/166947/00.pdf
Bycompanies <- rbind.data.frame(Bycompanies, c("Distributed","ENEOS Holdings Inc","operating","Parent",340,NA))
...
#Create Table S2
Bycompanies$Projects <- as.numeric(Bycompanies$Projects)
Bycompanies0 <- Bycompanies %>% group_by(Parent,Technology) %>% summarise(EqCapacity = sum(EqCapacity),
                                                                          Projects = sum(Projects))
# Pivot the table to have technologies as separate columns
output_table <- Bycompanies0 %>%
  pivot_wider(
    id_cols = Parent,
    names_from = Technology,
    values_from = c(EqCapacity, Projects),
)

output_table[,2:7] <- output_table[,2:7]/1000
for(i in 1:nrow(output_table)){
  output_table$EqCapacity[i] <- sum(output_table[i,2:7], na.rm = TRUE)
  output_table$Projects[i] <- sum(output_table[i,8:13], na.rm = TRUE) 
}

output_table <- output_table[order(output_table$EqCapacity, decreasing = TRUE),]

write.csv(output_table,"Ranking.csv")

Bycompanies1 <- Bycompanies %>% group_by(Parent,Company.Type) %>% summarise(EqCapacity = sum(EqCapacity),
                                                                          Projects = sum(Projects))
# Pivot the table to have technologies as separate columns
output_table_type <- Bycompanies1 %>%
  pivot_wider(
    id_cols = Parent,
    names_from = Company.Type,
    values_from = c(EqCapacity, Projects),
  )

output_table_type[,2:4] <- output_table_type[,2:4]/1000
for(i in 1:nrow(output_table_type)){
  output_table_type$EqCapacity[i] <- sum(output_table_type[i,2:7], na.rm = TRUE)
  output_table_type$Projects[i] <- sum(output_table_type[i,5:7], na.rm = TRUE) 
}

output_table_type <- output_table_type[order(output_table_type$EqCapacity, decreasing = TRUE),]

write.csv(output_table_type,"Ranking_type.csv")
...
#Plot ranking coloured by type of company (Figure S1)
...
Projects <- data_FF %>% group_by(Technology,CatCapacity) %>% summarise(Projects = sum(Projects))

write.csv(data_FF,"Data_FigureS3.csv")
write.csv(Projects,"Project counts FigureS3.csv")
...
Tab <- ownership_table(Status = "operating",Company.Type = c("Parent","Subsidiary","Acquisition"))
write_clip(Tab)
table(FF250$Status)
...
setwd("G:/My Drive/Research/ZZ.Submitted/Part of the Solution/Submission NS/Review #1/Data Analysis/PJ Piecharts")

#Create individual piechart plots for each of the 49 companies with renewable assets
#WARNING: Creating the company piecharts can take some time
for(i in which(a$Company %in% c(unique(Bycompanies$Parent),"Total"))){
  c <- b[((i*2)-1):(i*2),]
